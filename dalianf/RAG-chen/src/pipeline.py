...
class Pipeline:
    ...
    def download_docling_models():
        """Download required Docling models using a real PDF file."""
        import logging
        from pathlib import Path
        import os
        import gc
        ...
        try:
            ...
            # Use real PDF from the project's pdf_reports directory
            pdf_file = "中芯国际机构调研纪要.pdf"
            pdf_path = here() / "data" / "stock_data" / "pdf_reports" / pdf_file
            
            if not pdf_path.exists():
                raise FileNotFoundError(f"PDF file not found: {pdf_path}")
                
            # Create output directory for parsed results
            output_dir = here() / "data" / "stock_data" / "parsed_models_output"
            output_dir.mkdir(parents=True, exist_ok=True)
            
            logger.info(f"Using real PDF file: {pdf_path}")
            logger.info(f"Parsed results will be saved to: {output_dir}")
            
            # Initialize parser with minimal settings
            from src.pdf_parsing import PDFParser
            parser = PDFParser(output_dir=output_dir)
            parser.batch_size = 1
            parser.max_workers = 1
            parser.debug_mode = False
            parser.save_debug_data = False
            
            logger.info("Initializing model download and parsing PDF...")
            parser.parse_and_export([pdf_path])
            
            logger.info("Model download and PDF parsing completed successfully")
            logger.info(f"Parsed results saved in: {output_dir}")
            
        except Exception as e:
            logger.error(f"Error during model download/parsing: {str(e)}")
            raise
        finally:
            gc.collect()
